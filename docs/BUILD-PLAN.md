...
| M11a | **Debug scaffolding added, extends the M7a/M7b strip-list.** `DEBUG_KEYS` (`c` = player win, `v` = player loss, `[` / `]` step `ball_scale`) and `DEBUG_AI` (COM plays from the first serve, bypassing Denial's 2-point arming and its score reset). `DEBUG_KEYS` calls `poke(0x5f2d, 1)`, which enables devkit keyboard — it shows a warning banner to BBS players and does not exist on handheld or mobile targets, exactly what CLAUDE.md's PICO-8 rules forbid. **It must not ship `true`.** Both are single-flag kills |
