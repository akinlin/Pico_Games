...
| Rendering | `palette` (16-byte table), `nickname`, `phosphor_enabled` | Per-act colors; phosphor off in Anger |
...
|---|---|
| Per-act palette | **Two** precomputed 16-byte tables per act — the base display palette and the darkened scanline palette — applied with `memcpy(0x5f10, addr, 16)` and `memcpy(0x5f60, addr2, 16)`. One call each, ~5 tokens. Because they target the *display* palette they recolor the entire frame including already-drawn pixels, at zero per-object cost. Both must be swapped together on act load, or the scanlines will keep the previous act's colors |
| CRT scanlines | Per-line palette hardware: `poke(0x5f5f, 0x10)`, scanline palette at `0x5f60`–`0x5f6f`, per-line selection bitfield at `0x5f70`–`0x5f7f`. Set once and applied by the display hardware at no per-frame cost. Always on. Two caveats: this shares `0x5f60`–`0x5f6f` with `fillp`'s secondary palette, so the two cannot both be used; and **these addresses are undocumented in the PICO-8 manual**, so the behavior should be re-verified against any future release |
| Phosphor glow | Frame-blending via a stashed previous frame in remapped video memory. Costs roughly a quarter of the frame budget, so it is a per-act Customization axis: enabled in Denial, Bargaining, Depression and Acceptance; disabled in Anger, where the frame goes to the swarm instead |
...
| Scanlines | ~0 | ~0% |
| Phosphor *(disabled in Anger)* | ~33,000 | ~24% |
